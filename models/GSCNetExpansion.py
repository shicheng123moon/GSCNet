...
class GeneralWiseSeparableConv(nn.Module):
    ...
    def forward(self, x):
        ...
        concat_tensor = torch.cat([split1, split2, split3], 1) + expansion_layer
...
class GSCNetExpansion(nn.Module):
    def __init__(self, input_height=224, input_width=224, in_channels=3, n_classes=1000, width_mult=1., dropout_ratio=0.2,
        ...
        super(GSCNetExpansion, self).__init__()
...
if __name__=="__main__":
    model = GSCNetExpansion(input_height=320, input_width=320, in_channels=3, n_classes=1000, width_mult=1., dropout_ratio=0.2,
                 use_batch_norm=True, onnx_compatible=False)
